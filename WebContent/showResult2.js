...
//*********     zheming
			...
			// p_11.innerHTML = "Flight Number";
			 ...
			 var validatetxt = document.createElement('p');
			 validatetxt.innerHTML="Are you sure that you want to purchase this tickets?";
			 var buttons = document.createElement('ul');
	         buttons.className = "cd-buttons";
	         var liYes = document.createElement('li');
	         var popupYes = document.createElement('a');
	         popupYes.href = "#0";
	         popupYes.className="cd-popup-yes";
	         popupYes.innerHTML="Yes";
	        // popupYes.addEventListener("click",purchaseTickets,false);
	         var liNo = document.createElement('li');
	         var popupNo = document.createElement('a');
	         popupNo.href = "#0";
	         popupNo.className="cd-popup-no";
	         popupNo.innerHTML="No";
	         popupNo.addEventListener("click",closeDialog,false);
	         var popupClose = document.createElement('a');
	         popupClose.href="#0";
	         popupClose.className="cd-popup-close img-replace";
...
//Yuzhou's Popuo Dialog
	     document.getElementById("Results-RightRow"+i).appendChild(popup);
	     popup.appendChild(container);
	     container.appendChild(validatetxt);
	     container.appendChild(buttons);
	     container.appendChild(popupClose);
	     buttons.appendChild(liYes);
	     buttons.appendChild(liNo);
	     liNo.appendChild(popupNo);
	     liYes.appendChild(popupYes);
	}
}

// it triggers the event to show up the purchase dialog
function showMessage(index) {
  $('.cd-popup').addClass('is-visible');
  var str = this.id;
  var num = parseInt(str.slice(-1));

  $('.cd-popup-yes').click(function(){
...
setInterval(checkHasFlight,1*100);
...
setInterval(sortByPrice,1*5*1000);
 ...
 /* Yuzhou
 Main function that contains the logic to paginate
 */ 
	$.fn.customPaginate = function(options)
		{  
			...
			paginationContainer.find("ul li").on('click',function refresh(){
...
/*
Create pagination based on .Flight class
*/
		function createPagination(divisions){
 			filterCheck();
		 // if($(".Flight").size()!=Flight_num){

			$( ".pagination" ).empty();
			
			   $(".pagination").customPaginate({
		
		          itemsToPaginate: divisions
			
	            });	
	            Flight_num = $('.Flight').size();	
		// }
			
			
        }
 setInterval(createPagination('.Flight'),1*1000);
...
// 	    var price_str1 = $(keySelector,a).text();
// 		var price_str2 = $(keySelector,b).text();
//         var price1 = Number(price_str1.substring(1,price_str1.length));
//         var price2 = Number(price_str2.substring(1,price_str2.length));
//
// 		alert(price1+'  '+price2);
//
//         return (price1 < price2) ? -1 : (price1 > price2) ? 1 : 0;
...
// 	alert(items.length);
// }
//setInterval(sortUsingNestedText($('#DisplaySearchResults'),$('.Flight'),"p.coach_price"),1*1000);
/*function showStop0(){
   
  if($("#checkbox_0").prop("checked") =="checked"){
	
var stop0_list = $(".Flight").filter(function(){
	return $(".stop",this).text() == "0 stopover";
}); 
...
var stop1_list = $(".Flight").filter(function(){
	return $(".stop",this).text() == "1 stopover";
}); 
