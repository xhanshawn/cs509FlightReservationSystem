...
public class Flight {
	...
	public LocalTime getDATime(boolean depart){
		...
		if(!time_parsed){
			dep_time = LocalTime.parseStringToLocalTime(dep_time_str);
			arr_time = LocalTime.parseStringToLocalTime(arr_time_str);
			time_parsed = true;
		}
	 ...
	 * Abbreviation name.
	 * 
	 * The reason the time zone system are calculated inside system is to avoid wasting time for 
	 * Http requests.
	 */
	public String getDAOffsetString(boolean depart){
		...
		String substr = str.substring(0, str.length()-8);
